...
Development:
- Copy `frontend/.env.example` to `frontend/.env` and set `VITE_REACT_API_URL` (e.g. `http://localhost:5000/api`).
- Start the development server: `npm run dev` (or `npm start`). The app runs on the Vite dev server (e.g. http://localhost:5173).

Production build:
- Set the API URL before building: `VITE_REACT_API_URL=https://your-api-domain.com/api` (or `https://your-api-domain.com`).
- Run `npm run build`. Serve the `dist` folder with your hosting (e.g. Nginx, Vercel).
- All API calls use the central config in `src/config.js`, which reads `VITE_REACT_API_URL` at build time.
