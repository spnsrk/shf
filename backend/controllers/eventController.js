...
exports.createEvent = async (req, res) => {
  ...
  const eventImageUrl = req.files?.eventImage?.[0]?.location || '';
  const eventPDFUrl = req.files?.eventPDF?.[0]?.location || '';
...
exports.updateEvent = async (req, res) => {
  ...
  try {
    ...
    if (req.files?.eventImage && event.eventImageUrl) {
    ...
    event.eventPDFUrl = eventPDFUrl || event.eventPDFUrl;
    ...
    event.eventTimeFrom = eventTimeFrom || event.eventTimeFrom;
    event.eventTimeTo = eventTimeTo || event.eventTimeTo;
